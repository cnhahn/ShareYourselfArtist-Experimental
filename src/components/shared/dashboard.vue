  ...
  methods: {
    ...
    searchArtistEmail() {
      ...
      for(let obj in artistList) {
        // console.log(artistList[obj])
        // console.log('search: ' + this.artist_email + ' current obj email: ' + obj.artist_email)
        if (this.artist_email == artistList[obj].artist_email) {
          
          //set store for artist found
          this.$store.dispatch('set_artist_settings_artist', artistList[obj])
          //redirect to artist_settings page
          this.$router.push('artist_settings.vue')
