...
export const store = new Vuex.Store({
  state: {
    ...
    submissions_for_month: [], //aortizoj
  ...
  mutations: {
    ...
    set_paid_submissions_for_month(state, payload) {
      state.paid_submissions_for_month.push(payload);
    },
  ...
  actions: {
    ...
    get_submissions_for_month({ commit, getters }) {
      ...
      let query = temp_report
        .where("replied_date", ">", epoch_firstDayOfMonth)
        .where("replied_date", "<", epoch_lastDayOfMonth);
      query
        .get()
    ...
    report_aug({ commit, getters }, payload) {
      ...
      let temp_report = db.collection("review_requests");
      let query = temp_report
        .where("businessId.business_email", "==", payload.business_email)
    ...
    report_nov({ commit, getters }, payload) {
      ...
      let query = temp_report
        .where("businessId.business_email", "==", payload.business_email)
        .where("replied_date", ">", 1538870400000)
        .where("replied_date", "<", 1540166400000);
      query
        .get()
        .then(function(results) {
          if (results.empty) {
